<?php
	/**
	 *	This files intented use is for creating a HTML file based on the TFplanner output.
	 *
	**/
	...
	/**
	 *	Function formatting and saves a HTML file from string. 
	 *
	 *	@param $svg: svg element, the image generated from raphael.
	 * 	@param $matTable: string created by javascript in html-format. containing the product-table.
	 *	@param $note: The additional note containing product genre specific information.
	**/
	function saveSVG($svg, $matTable, $note) {
		...
		**/
		...
		$saveAsFile = $dir.'\entry_'.$entryId.'.html';
			
		$htmlString = $bodystart.$head.$css.$background.$header.$image.$table.$AdditionalNote.$foot.$bodyend;

		// Saving string as file.
		file_put_contents($saveAsFile, $htmlString);		
